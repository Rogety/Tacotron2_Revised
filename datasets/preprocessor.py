...
def build_from_path(hparams, input_dirs, mel_dir, linear_dir, wav_dir, n_jobs=12, tqdm=lambda x: x):
	...
	for input_dir in input_dirs:
		with open(os.path.join(input_dir, 'arctic_sentense.txt'), encoding='utf-8') as f:
			for line in f:
				parts = line.strip().split('|')
				basename = "arctic" + re.search(pattern_name,line).group(0)
				wav_path = os.path.join(input_dir, 'wavs', '{}.wav'.format(basename))
				text = re.search(pattern_sen,line).group(0).strip('"')
				futures.append(executor.submit(partial(_process_utterance, mel_dir, linear_dir, wav_dir, basename, wav_path, text, hparams)))
				index += 1
...
def _process_utterance(mel_dir, linear_dir, wav_dir, index, wav_path, text, hparams):
	...
	return (audio_filename, mel_filename, linear_filename, time_steps, mel_frames, text, label_filename)
