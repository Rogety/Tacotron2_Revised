...
class Synthesizer:
	def load(self, checkpoint_path, hparams, gta=False, model_name='Tacotron'):
		...
		inputs = tf.placeholder(tf.float32, (None, None, 752), name='inputs')
		...
		print("input_lengths :",input_lengths.shape,input_lengths.dtype)
		print("targets :",targets.shape,targets.dtype)
		print("split_infos :",split_infos.shape,split_infos.dtype)
	...
	def synthesize(self, texts, basenames, out_dir, log_dir, mel_filenames, labels):
		...
		input_lengths = [seq.shape[0] for seq in seqs]
		...
		for i in range(self._hparams.tacotron_num_gpus):
			...
			device_input, max_seq_len = self._prepare_inputs_vector(device_input)
		...
		for i, mel in enumerate(mels):
			...
			if log_dir is not None:
				...
				#print("target_lengths[i] :",target_lengths[i])
				#print("alignments[i] :",alignments[i].shape)
				#save alignments
