...
class Feeder:
	...
	def __init__(self, coordinator, metadata_filename, hparams):
		...
		self._label_dir = os.path.join(os.path.dirname(metadata_filename), 'label')
		...
		with tf.device('/cpu:0'):
			...
			self._placeholders = [
			tf.placeholder(tf.float32, shape=(None, None, 752), name='inputs'),
			...
			queue = tf.FIFOQueue(8, [tf.float32, tf.int32, tf.float32, tf.float32, tf.float32, tf.int32, tf.int32], name='input_queue')
			...
			eval_queue = tf.FIFOQueue(1, [tf.float32, tf.int32, tf.float32, tf.float32, tf.float32, tf.int32, tf.int32], name='eval_queue')
	...
	def _get_test_groups(self):
		...
		input_data = np.load(os.path.join(self._label_dir, meta[6]))
	...
	def _prepare_batch(self, batches, outputs_per_step):
		...
		for i in range(self._hparams.tacotron_num_gpus):
			...
			input_cur_device, input_max_len = self._prepare_inputs_vector([x[0] for x in batch])
	...
	def _prepare_inputs_vector(self, inputs):
		max_len = max(len(inputs[idx]) for idx in range(len(inputs)))
		return np.stack([self._pad_input_vector(x, max_len) for x in inputs]), max_len
	...
	def _pad_input_vector(self, x, max_len):
		return np.pad(x , ((0,max_len - len(x)), (0,0)) , mode='constant', constant_values=self._pad)
