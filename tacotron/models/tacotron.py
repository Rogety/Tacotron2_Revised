...
class Tacotron():
	...
	def initialize(self, inputs, input_lengths, mel_targets=None, stop_token_targets=None, linear_targets=None, targets_lengths=None, gta=False,
		...
		print("inputs :",type(inputs),inputs.shape,inputs.dtype)
		...
		with tf.device(split_device):
			...
			print("p_inputs :",type(p_inputs))
			batch_size = tf.shape(inputs)[0]
			mel_channels = hp.num_mels
			linear_channels = hp.num_freq
			for i in range (hp.tacotron_num_gpus):
				tower_inputs.append(tf.reshape(p_inputs[i], [batch_size, -1, 752]))
		...
		print("tower_inputs :",type(tower_inputs),len(tower_inputs),tower_inputs[0].shape)
		print("tower_input_lengths :",type(tower_input_lengths),len(tower_input_lengths),type(tower_input_lengths[0]),tower_input_lengths[0].dtype)
		#print("mel_targets :",type(mel_targets),mel_targets.shape)
		...
		for i in range(hp.tacotron_num_gpus):
			with tf.device(tf.train.replica_device_setter(ps_tasks=1, ps_device="/cpu:0", worker_device=gpus[i])):
				with tf.variable_scope('inference') as scope:
					...
					print("embedded_inputs :",embedded_inputs.shape,embedded_inputs.dtype)
					print("tower_input_lengths[i] :",tower_input_lengths[i].shape,tower_input_lengths[i].dtype)
					#Attention Mechanism
					attention_mechanism = LocationSensitiveAttention(hp.attention_dim, embedded_inputs, hparams=hp, is_training=is_training,
