...
def run_eval(args, checkpoint_path, output_dir, hparams, sentences):
	...
	sentences_dir = os.path.join("training_data", "sentences")
	...
	labels = [np.load(os.path.join("training_data", "sentences", x) ) for x in sorted(os.listdir(sentences_dir))]
	...
	with open(os.path.join(eval_dir, 'map.txt'), 'w') as file:
		for i, texts in enumerate(tqdm(sentences)):
			...
			mel_filenames, speaker_ids = synth.synthesize(texts, basenames, eval_dir, log_dir, None, labels[i])
...
def run_synthesis(args, checkpoint_path, output_dir, hparams):
	...
	lab_dir = os.path.join(args.input_dir, 'label')
	with open(os.path.join(synth_dir, 'map.txt'), 'w') as file:
		for i, meta in enumerate(tqdm(metadata)):
			texts = [m[5] for m in meta]
			mel_filenames = [os.path.join(mel_dir, m[1]) for m in meta]
			wav_filenames = [os.path.join(wav_dir, m[0]) for m in meta]
			label_filenames = [os.path.join(lab_dir, m[6]) for m in meta]
			basenames = [os.path.basename(m).replace('.npy', '').replace('mel-', '') for m in mel_filenames]
			print("label_filenames :",label_filenames)
			labels = np.load(label_filenames[0]) 
			mel_output_filenames, speaker_ids = synth.synthesize(texts, basenames, synth_dir, None, mel_filenames, labels)
